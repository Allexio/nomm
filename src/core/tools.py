...
def translate_fuse_path(folder_info) -> str:
    ...
    if "run/user" in folder_path:
        ...
        try:
            ...
            file_info = folder_info.query_info(
                "xattr::document-portal.host-path", Gio.FileQueryInfoFlags.NONE, None
            )

            # Query file attribute for real path
            real_path = file_info.get_attribute_string(
            ...
            print(
                "Can not get real path. If you see this message you will need to manually give NOMM host filesystem permissions."
            )
    return folder_path
...
def retrieve_casesensitive_paths(path: str):
    ...
    for part in parts[1:]:
        try:
            ...
            return None
        found_item = next(
            (f for f in os.listdir(new_path) if f.lower() == part.lower()), None
        )
        if found_item:
        ...
        try:
            ...
            if mime_type in ARCHIVE_MIME_TYPES:
                ...
                if "yaml" not in mime_type:
                    print(
                        f"[!] Could not identify mime type in download folder: {mime_type}"
                    )
        except Exception as e:
...
def launch_option_merger(current_launch_options: str, new_option: str) -> str:
    if new_option.strip() in current_launch_options:
        return current_launch_options

    if "%command%" in current_launch_options and "%command%" in new_option:
        current_prefix, current_suffix = current_launch_options.split("%command%", 1)
        new_prefix, new_suffix = new_option.split("%command%", 1)

        merged_prefix = " ".join(
            part.strip() for part in (current_prefix, new_prefix) if part.strip()
        )
        merged_suffix = " ".join(
            part.strip() for part in (current_suffix, new_suffix) if part.strip()
        )
    ...
    if "%command%" in current_launch_options and "%command%" not in new_option:
        current_prefix, _current_suffix = current_launch_options.split("%command%", 1)
        current_prefix = current_prefix.strip()
        merged = (
            (current_prefix + " " + new_option).strip()
            if current_prefix
            else new_option.strip()
        )
