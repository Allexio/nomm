...
from core.tools import load_yaml, translate_fuse_path, write_yaml
from core.user_config import load_user_config, update_user_config, write_user_config
...
class Nomm(Adw.Application):
    def __init__(self, **kwargs):
        ...
        super().__init__(
            application_id=APP_NAME, flags=Gio.ApplicationFlags.HANDLES_OPEN, **kwargs
        )
        self.matches = []
        ...
        if os.path.exists(os.path.join(os.path.dirname(base_path), "assets")):
            ...
            self.default_game_config_path = os.path.join(
                os.path.dirname(base_path), "default_game_configs"
            )
        else:
            self.assets_path = os.path.join(base_path, "assets")
            self.default_game_config_path = os.path.join(
                base_path, "default_game_configs"
            )
    ...
    def do_open(self, files, n_files, hint):
        for f in files:
            ...
            self.hold()
            threading.Thread(
                target=self._process_nxm_link, args=(uri,), daemon=True
            ).start()
        ...
        for filename in os.listdir(src):
            if filename.lower().endswith((".yaml", ".yml")):
                try:
                    ...
                    shutil.copy2(
                        os.path.join(src, filename), os.path.join(dest, filename)
                    )
                except:
                    pass
    ...
    def show_staging_select_screen(self):
        ...
        warning_label.set_markup(
            _(
                "<b>Important:</b> If using Flatpaks for your platforms (Steam, Heroic, etc.), ensure they have permission to access this folder (you can do this via command line or Flatseal)."
            )
        )
    ...
    def show_nexus_api_key_screen(self):
        ...
        status_page = Adw.StatusPage(
            ...
            description=_(
                "If you want to download mods from Nexus Mods, enter your API Key (Site Preferences > API Keys > scroll all the way down)"
            ),
        ...
        self.api_entry = Gtk.Entry(
            placeholder_text=_("Enter API Key..."), width_request=400, visibility=False
        )

        cont_btn = Gtk.Button(label=_("Continue"))
        cont_btn.add_css_class("suggested-action")
        cont_btn.connect(
            "clicked", lambda b: self.store_api_key(self.api_entry.get_text())
        )
    ...
    def steam_user_id_handler(self):
        ...
        steam_user_ids = [
            f
            for f in os.listdir(steam_userdata_path)
    ...
    def show_steam_user_id_selection_screen(self, steam_user_ids):
        status_page = Adw.StatusPage(
            ...
            description=_(
                "Multiple Steam user IDs were detected in your Steam installation.\n"
        ...
        for steam_user_id in steam_user_ids:
            options.append(
                {
                    "id": steam_user_id,
                    "username": get_username_from_steam_id(
                        steam_user_id, self.steam_base
                    ),
                }
            )
    ...
    def show_loading_and_scan(self):
        ...
        box = Gtk.Box(
            orientation=Gtk.Orientation.VERTICAL, spacing=30, valign=Gtk.Align.CENTER
        )
        spinner = Gtk.Spinner()
    ...
    def run_background_workflow(self):
        ...
        self.locked_essential_paths = [
            path for path in essential_paths if not os.access(path, os.W_OK)
        ]

        # Check which game libraries are locked (No Write Access)
        print(f"Checking for access rights to library paths: {game_libraries}")
        self.locked_libraries = [
            lib for lib in game_libraries if not os.access(lib, os.W_OK)
        ]
        ...
        if "ignored_libraries" in user_config:
            ...
            self.locked_libraries = [
                path for path in self.locked_libraries if path not in ignored_libraries
            ]
        ...
        if self.locked_libraries or self.locked_essential_paths:
            print(
                f"Missing read/write access to some paths: {str(self.locked_libraries + self.locked_essential_paths)}"
            )
    ...
    def show_permission_request(self):
        status_page = Adw.StatusPage(
            ...
            description=_(
                "NOMM needs some extra permissions to read/write to specific folders.\n"
        ...
        paths_str = " ".join(
            [
                f"--filesystem='{p}'"
                for p in (self.locked_libraries + self.locked_essential_paths)
            ]
        )
        ...
        )  # Essential for wrapping long paths
        ...
        copy_btn = Gtk.Button(
            icon_name="edit-copy-symbolic", tooltip_text=_("Copy to Clipboard")
        )
        copy_btn.set_valign(
            Gtk.Align.START
        )  # Keep button at the top of the multi-line block
    ...
    def show_library_ui(self):
        ...
        if user_config.get("enable_launcher_skip") and user_config.get(
            "last_selected_game"
        ):
            game_info = next(
                (
                    ...
                    if m["name"] == user_config.get("last_selected_game")
                ),
                None,
            )
            if game_info:
    ...
    def on_game_clicked(self, game_data):
        ...
        if config.get("download_path"):
            os.makedirs(
                os.path.join(config.get("download_path"), game_data["name"]),
                exist_ok=True,
            )
    ...
    def open_dashboard(self, game_info):
        self.dashboard = GameDashboard(application=self, game_info=game_info)
